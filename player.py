...
class Player:
    def __init__(self, name,  in_row, color):
        self.name =name
        self.points  = 0
        self.spells = []
        self.in_row=in_row
    ...
    def get_pixels(self):
        ...
        width=0
        if self.in_row:
            width = max(len(alphabet.translate(str(self.points))[0]), len(alphabet.translate('000')[0]))
            arr=library.concat_words(alphabet.translate_with_color(self.name+': ',self.color),alphabet.translate(str(self.points)))
        else:
        ...
        if not self.in_row:
            ...
            arr_points = alphabet.translate(str(self.points))
